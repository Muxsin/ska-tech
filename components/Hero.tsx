...
const Hero: React.FC = () => {
  ...
  return (
    <section className="relative pt-32 pb-20 lg:pt-48 lg:pb-32 overflow-hidden min-h-[90vh] flex flex-col justify-center transition-colors duration-300">
      ...
      {/* 2. Ambient Gradient Orbs (Brand & Indigo) - Static & Lightweight */}
      <div className="absolute inset-0 pointer-events-none overflow-hidden transform-gpu">
        ...
        {/* Blobs - Static and Optimized */}
        <div className="absolute top-[-10%] left-1/2 -translate-x-1/2 w-[150%] h-[1200px] opacity-40 dark:opacity-50">
           <div className="absolute top-[20%] left-[35%] w-[600px] h-[600px] bg-brand/10 rounded-full mix-blend-multiply dark:mix-blend-screen filter blur-[80px]"></div>
           <div className="absolute top-[25%] right-[35%] w-[500px] h-[500px] bg-indigo-500/10 rounded-full mix-blend-multiply dark:mix-blend-screen filter blur-[80px]"></div>
           <div className="absolute bottom-[20%] left-[45%] w-[600px] h-[600px] bg-indigo-400/10 rounded-full mix-blend-multiply dark:mix-blend-screen filter blur-[80px]"></div>
        </div>
      </div>

      {/* 3. Horizon Beam (Tech Arc) - Static */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full max-w-[1200px] pointer-events-none">
        <div className="relative w-full h-[300px]">
          <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-[2px] bg-brand/40 dark:bg-brand/60 blur-[20px]"></div>
      ...
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full h-full pointer-events-none overflow-hidden flex items-start justify-center pt-20">
        <div className="relative w-[700px] h-[700px] md:w-[1000px] md:h-[1000px] opacity-[0.15]">
          <div className="absolute inset-0 border border-slate-400/30 dark:border-brand/30 rounded-full"></div>
          <div className="absolute inset-[10%] border border-slate-400/20 dark:border-brand/20 border-dashed rounded-full"></div>
          <div className="absolute inset-[25%] border border-slate-400/10 dark:border-brand/10 border-dotted rounded-full"></div>
        </div>
      </div>
      
      {/* --- CONTENT (z-10) --- */}
      <div className="max-w-6xl mx-auto px-6 text-center relative z-10 w-full">
        <div className="inline-flex items-center px-4 py-1.5 rounded-full border border-slate-200 dark:border-white/10 bg-white/50 dark:bg-white/5 text-[11px] font-bold uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400 mb-10 backdrop-blur-md relative overflow-hidden max-w-full">
          <div className="absolute inset-0 bg-gradient-to-r from-transparent via-brand/10 to-transparent" style={{ backgroundSize: '200% 100%' }}></div>
          <span className="text-brand-dark dark:text-brand mr-2 truncate">{t('hero.new')}</span> 
          <span className="truncate">{t('hero.badge')}</span> 
          <ArrowRight className="ml-2 w-3.5 h-3.5 flex-shrink-0" />
        </div>
        
        <h1 className="text-4xl sm:text-5xl md:text-8xl font-extrabold tracking-tighter text-slate-900 dark:text-white mb-10 leading-[0.95] drop-shadow-xl dark:drop-shadow-[0_15px_40px_rgba(0,0,0,0.8)] break-words hyphens-auto">
          {t('hero.title')} <br /> 
          <span className="text-brand-dark dark:text-brand">{t('hero.titleHighlight')}</span> {t('hero.titleSuffix')}
        </h1>
        
        <p className="text-base md:text-lg text-slate-600 dark:text-slate-400 max-w-2xl mx-auto mb-14 leading-relaxed font-medium break-words">
          {t('hero.desc')}
        </p>
      ...
      <div className="relative w-full max-w-[90rem] mx-auto px-4 sm:px-6 lg:px-8 z-10">
        <div className="relative group">
          {/* Static Blur behind image - Optimized */}
          <div className="absolute -top-10 left-1/2 -translate-x-1/2 w-4/5 h-[200px] bg-brand/20 blur-[60px] rounded-full pointer-events-none opacity-40"></div>
